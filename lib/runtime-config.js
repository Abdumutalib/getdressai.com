...
function isSupabaseHttpUrl(value) {
...
function getRuntimeConfig() {
  ...
  const allowFileStoreInProduction = String(process.env.ALLOW_FILE_STORE_IN_PRODUCTION || "")
    .trim()
    .toLowerCase() === "true";
  ...
  if (hasSupabaseConfig && !isSupabaseHttpUrl(process.env.SUPABASE_URL)) {
    throw new Error(
      "SUPABASE_URL must be the HTTPS project URL, for example https://your-project.supabase.co, not a Postgres connection string."
    );
  }

  if (isProduction && userStoreDriver === "file" && !allowFileStoreInProduction) {
    throw new Error(
      "File-based user storage is disabled in production unless ALLOW_FILE_STORE_IN_PRODUCTION=true. Configure Supabase/PostgreSQL credentials instead."
    );
  }
