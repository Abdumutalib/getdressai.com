...
// PRO motivational text
const proMotivation = "Unlock better styles 🔒";
const proActive = "Premium AI active 🔥";
// IAP state
const [purchases, setPurchases] = useState([]);
// IAP connection and purchase check
useEffect(() => {
  RNIap.initConnection();
  RNIap.getAvailablePurchases().then(res => setPurchases(res)).catch(() => {});
  return () => { RNIap.endConnection(); };
}, []);

useEffect(() => {
  if (purchases.length > 0) setUser(u => ({ ...u, plan: "pro" }));
}, [purchases]);
// Buy PRO handler
const buyPro = async () => {
  try {
...
import * as Sharing from 'expo-sharing';
  // Referral code (user ID ёки email асосида)
  const referralCode = user?.id || user?.email || "GUEST";
  const referralLink = `https://dressai.app/invite?ref=${encodeURIComponent(referralCode)}`;
      ...
      await Sharing.shareAsync(undefined, {
        ...
        <View style={styles.card}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 10 }}>
            ...
            <TouchableOpacity onPress={() => { Clipboard.setString(referralCode); Alert.alert('Copied!'); }} style={{ backgroundColor: '#f1e7d9', borderRadius: 8, padding: 6 }}>
              <Text style={{ color: '#c94f3d', fontWeight: 'bold' }}>Copy</Text>
            </TouchableOpacity>
          </View>
          <TouchableOpacity onPress={handleShareReferral} style={{ backgroundColor: '#c94f3d', borderRadius: 8, padding: 10, alignItems: 'center' }}>
...
const API = "http://192.168.1.104:3000";
...
export default function App() {
  ...
  const generateOutfit = async () => {
    ...
    try {
      ...
      // Affiliate shop: recommendations
      if (response.data?.recommendations?.products) {
        setRecommendations(response.data.recommendations.products);
      } else {
        setRecommendations([]);
      }
      if (response.data?.user) {
        ...
        await persistGuestUsage(nextGuestUsage);
            ...
            {/* Affiliate Shop: Outfit Recommendations */}
            {recommendations.length > 0 && (
              <View style={styles.card}>
                <Text style={styles.cardTitle}>Shop the Look</Text>
                <FlatList
                  data={recommendations}
                  keyExtractor={(item, idx) => item.affiliateUrl || item.url || String(idx)}
                  renderItem={({ item }) => (
                    <View style={{ marginBottom: 16, borderBottomWidth: 1, borderColor: '#eadfce', paddingBottom: 10 }}>
                      <Text style={{ fontWeight: 'bold', fontSize: 15 }}>{item.name || item.title || 'Product'}</Text>
                      {item.image && (
                        <Image source={{ uri: item.image }} style={{ width: '100%', height: 120, borderRadius: 10, marginVertical: 6 }} resizeMode="cover" />
                      )}
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        {isLoading ? (
          <View style={styles.loadingContainer}>
            <HangerPremiumSpin size={64} color="#c94f3d" />
